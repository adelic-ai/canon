...
**a growing set of producers wired and tested**, spanning the feature axis (`count`, `entropy`,
`KL`, `MI`, `distinct-count`, `HHI`) and the test axis (`CFAR`, `conformal`), each producing a
verdict that validates against the PINNED `contracts/detection_verdict.schema.json`. The vertical
slice composes across both axes. The real gap is Axis-A **breadth, and it is descriptive statistics, not IT**:
...
- The detection battery's **vertical slice** (feature → test → five-fold verdict → PINNED
  contract) is **built and tested** across the feature axis (count / entropy / KL / MI /
  distinct-count / HHI) × the test axis (CFAR / conformal); the full feature × test grid and any
