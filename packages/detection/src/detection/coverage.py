...
def coverage_report(rules: list[dict], compiled: list[CompiledRule], lattice: dict, techniques) -> dict:
    """The per-TTP structural coverage report. ``rules`` are the raw (tagged) evaluable dicts, ``compiled``
    their IRs, ``lattice`` the stage-3 relation lattice, ``techniques`` the requested scope."""
    ...
    return {
        ...
        "per_technique": per,
        "layers": ["primitive", "atom", "rule", "concept"],  # catch-class deferred to stage 4 (grounding)
        "caveat": "structural coverage only; concepts=exactMatch dedup (over-collapse-biased upper bound); "
                  "behavioral redundancy (purposeless dup vs independent redundant witness) awaits catch-set",
    }
