...
def test_detects_upward_shift():
    ...
    out = cusum_op(Signal(samples=x, fs=1.0), target=0.0, sigma=1.0, k=0.5, h=5.0)
...
def test_detects_downward_shift_direction():
    ...
    out = cusum_op(Signal(samples=x, fs=1.0), target=0.0, sigma=1.0)
    first = out["indices"][0]
...
def test_reset_detects_both_directions():
    ...
    out = cusum_op(s, target=0.0, sigma=1.0, reset=True)
...
def test_robust_estimation_recovers_scale():
    ...
    out = cusum_op(Signal(samples=x, fs=1.0))  # no target/sigma given
